...
def check_auto_save(content):
    """Check that auto-save via localStorage is present."""
    has_localstorage = bool(re.search(r"localStorage", content))
    has_autosave_fn = bool(re.search(r"autoSave|_doSave|doSave", content))
    has_restore = bool(re.search(r"restoreState|loadState", content))

    present = []
    missing = []
    for name, val in [("localStorage", has_localstorage), ("autoSave function", has_autosave_fn), ("restore function", has_restore)]:
        (present if val else missing).append(name)

    return {
        "metric": "auto_save_present",
        "passed": len(missing) == 0,
        "details": f"Present: {', '.join(present)}" + (f". Missing: {', '.join(missing)}" if missing else ""),
    }


def check_undo_redo(content):
    """Check that undo/redo functionality is present."""
    has_undo_fn = bool(re.search(r"function\s+undo\b", content))
    has_redo_fn = bool(re.search(r"function\s+redo\b", content))
    has_save_undo = bool(re.search(r"saveUndo|pushUndo", content))
    has_keybind = bool(re.search(r"Ctrl.*Z|ctrlKey.*[Zz]|metaKey.*[Zz]", content))

    present = []
    missing = []
    for name, val in [("undo()", has_undo_fn), ("redo()", has_redo_fn), ("saveUndo()", has_save_undo), ("keyboard binding", has_keybind)]:
        (present if val else missing).append(name)

    return {
        "metric": "undo_redo_present",
        "passed": len(missing) == 0,
        "details": f"Present: {', '.join(present)}" + (f". Missing: {', '.join(missing)}" if missing else ""),
    }


def check_stats_dashboard(content):
    """Check that stats dashboard is present."""
    has_stats_el = bool(re.search(r"stat-card|stats-bar|statsBar", content))
    has_update_fn = bool(re.search(r"updateStats|renderStats", content))
    has_duration = bool(re.search(r"duration|Duration", content))

    present = []
    missing = []
    for name, val in [("stats UI", has_stats_el), ("updateStats function", has_update_fn), ("duration display", has_duration)]:
        (present if val else missing).append(name)

    return {
        "metric": "stats_dashboard_present",
        "passed": len(missing) == 0,
        "details": f"Present: {', '.join(present)}" + (f". Missing: {', '.join(missing)}" if missing else ""),
    }


def check_filter_bar(content):
    """Check that filter bar is present."""
    has_filter_ui = bool(re.search(r"filter-bar|filterBar|filter-btn", content))
    has_filter_fn = bool(re.search(r"setFilter|applyFilter", content))
    has_search = bool(re.search(r"searchInput|search-input|filterSearch", content))

    present = []
    missing = []
    for name, val in [("filter UI", has_filter_ui), ("filter function", has_filter_fn), ("search input", has_search)]:
        (present if val else missing).append(name)

    return {
        "metric": "filter_bar_present",
        "passed": len(missing) == 0,
        "details": f"Present: {', '.join(present)}" + (f". Missing: {', '.join(missing)}" if missing else ""),
    }


def check_keyboard_shortcuts(content):
    """Check that keyboard shortcuts are present."""
    has_keydown = bool(re.search(r"keydown|onkeydown", content))
    has_ctrl_z = bool(re.search(r"[Zz].*undo|undo.*[Zz]|ctrlKey.*[Zz]|metaKey.*[Zz]", content, re.DOTALL))
    has_space = bool(re.search(r"Space|space.*play|togglePlay", content))

    present = []
    missing = []
    for name, val in [("keydown listener", has_keydown), ("Ctrl+Z binding", has_ctrl_z), ("Space play/pause", has_space)]:
        (present if val else missing).append(name)

    return {
        "metric": "keyboard_shortcuts_present",
        "passed": len(missing) == 0,
        "details": f"Present: {', '.join(present)}" + (f". Missing: {', '.join(missing)}" if missing else ""),
    }
...
def main():
    ...
    checks = [
        ...
        check_auto_save(content),
        check_undo_redo(content),
        check_stats_dashboard(content),
        check_filter_bar(content),
        check_keyboard_shortcuts(content),
    ]
